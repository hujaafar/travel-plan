...
- Git initially failed certificate verification. Using the existing trusted roots restored access; this checkout now uses Windows certificate trust for the course host. The complete source history was scanned for generated secrets/private paths, then `feature/admin-platform` was pushed and its remote commit verified. The server emitted a hook warning on the first push, but the branch was confirmed through `ls-remote`. The repository API returned 403 and browser automation could not initialize; no remote PR or branch protection was configured, and no independent approval or merge is claimed.
