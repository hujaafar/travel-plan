...
The first fresh cloud startup exposed a real resource defect: the Vault server was killed by Docker at its 192 MB memory limit (exit 137, OOMKilled true). The default server limit is now 512 MB, agents are 256 MB, and Go memory/concurrency targets are set explicitly. A following run progressed past provisioning into the real infrastructure checks and Jenkins pipeline. Final check results must be taken from the current GitHub PR; failed earlier runs remain visible. This change does not claim database/host high availability.
