...
| --- | --- | --- |
| Java services | Two instances per service in base Compose, Caddy DNS discovery, passive upstream failure handling, shared sessions; explicit failure probe | Execute the probe on a healthy two-replica environment and preserve measured results. A successful local test still cannot prove host-level availability. |
...
Docker was reinstalled on 14 September, and Engine 29.7.2 was verified responsive. The old Docker database disk was lost during uninstall; no Travel Plan data export was available. After the Maven trust fix, startup reached fresh PostgreSQL, then the Travel stack was stopped to leave the runtime available for the user's Neo4flix viewing session. The current 38 configuration contracts pass; a full post-reinstall runtime, Ansible deployment, CI, logging and failover gate remains unverified. Earlier successful bootstrap/build and Linux syntax results are historical evidence, not post-reinstall results. See DOCKER-RECHECK.md. Kubernetes remains optional.
