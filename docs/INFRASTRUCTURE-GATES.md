...
| Jenkins and review | Pipeline source, isolated build model, unit/config checks, main-only Community Sonar gate, protected deployment stage | The isolated local candidate agent and Sonar gate passed. PR #1 and main protection are now configured; the exact required `travel-plan/jenkins` check and one approval are still missing. Connect the full PR/deployment agent and SCM/status publisher, and obtain independent approval. The current Community gate does not analyze PRs before merging; use a supported offering if pre-merge Sonar analysis is required. |
